  ...
  Mutation: {
    insertCategory: async (parent, args, context, info) => {
      try {
        ...
        const data = await db("categories").insert(entity).returning("*");
        return {
          result: [data],
        };
      } catch (error) {
    ...
    updateCategory: async (parent, args, context, info) => {
      try {
        ...
        const data = await db("categories").where({ id }).update(entity).returning("*");
        return {
          result: data,
        };
      } catch (error) {
    ...
    deleteCategory: async (parent, args, context, info) => {
      try {
        const { id } = args;
        const data = await db("categories").del().where({ id }).returning("*");
        return {
          result: data,
        };
      } catch (error) {
